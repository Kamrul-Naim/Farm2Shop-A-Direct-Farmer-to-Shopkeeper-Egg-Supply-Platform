...
const createOrder = async (req, res) => {
    try {
      ...
      const shopkeeper = await shopkeeperModel.findById(req.user.id);
      ...
      // Only verified shopkeepers can order
      ...
      if (quantity < product.minimumOrderQuantity) {
        return res.status(400).json({
          success: false,
          message: `Minimum order quantity is ${product.minimumOrderQuantity} pieces.`,
        });
      }
      ...
      if (quantity > product.quantity) {
        return res.status(400).json({
          success: false,
          message: `Only ${product.quantity} pieces are available.`,
        });
      }

      // Get category and current price
      const category = await categoryPriceModel.findById(product.category);
      ...
      // If no stock remains, mark unavailable
      ...
      await createNotification({
        ...
        message: `You received a new order for ${quantity} pieces of ${product.productName}.`,
        order: order._id,
      });
      ...
      await createNotification({
        ...
        title: "Order Placed",
        message: `Your order for ${quantity} pieces of ${product.productName} has been placed successfully.`,
        order: order._id,
      });
